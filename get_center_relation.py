...
labe_list_path='/home/yangshuhui/code/data/label_list.txt'

with open(labe_list_path) as f:
	img_paths = f.readlines()

center_list=open("/home/yangshuhui/code/data/center_list.txt","wb")

for path in img_paths:
	centers = get_centers(path.rstrip())
        center_list.write(pix_center['imagename']+'!')
	for key,values in pix_center.items():
		if key=='imagename':
			continue
		else:
			center_list.write(key+':')
			coordinate=''.join(str(values))
			center_list.write(coordinate+';')
	center_list.write('\n')

classes={'road':[128,64,128],  'sidewalk':[244,35,232],      'building':[70,70,70],      'wall':[102,102,156],      'fence':[190,153,153],\
         'pole':[153,153,153], 'traffic light':[250,170,30], 'traffic sign':[220,220,0], 'vegetation':[107,142,35], 'terrain':[152,251,152],\
         'sky':[70,130,180],   'person':[220,20,60],         'rider':[255,0,0],          'car':[0,0,142],           'truck':[0,0,70],\
         'bus':[0,60,100],     'train':[0,80,100],           'motorcycle':[0,0,230],     'bicycle':[119,11,32]}


def get_centers(imgaddress):
